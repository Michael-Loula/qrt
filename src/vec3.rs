...
//macro for easy construction of vec3 using int or decimal values
#[macro_export]
macro_rules! v3 {
...
impl Hittable for HittableList {
    fn hit(&self, r: Ray, t_0: f64, t_1: f64) -> Option<HitRecord> {
        let mut hr: HitRecord = HitRecord {
            point: v3!(0, 0, 0),
            normal: v3!(0, 0, 0),
            t: 0.0,
            front_face: false,
...
impl Camera {
    pub fn get_ray(&self, u: f64, v: f64) -> Ray {
        Ray {
            origin: self.origin,
            direction: self.llc + self.horiz * u + self.vert * v - self.origin,
        }
    }
}
...
impl Material for Lambertian {
    fn scatter(&self, _r: Ray, hr: &HitRecord) -> (Vec3, Ray, bool) {
...
impl Material for Metal {
    fn scatter(&self, r: Ray, hr: &HitRecord) -> (Vec3, Ray, bool) {
        ...
        (
            self.albedo,
            scatter,
            Vec3::dot(scatter.direction, hr.normal) > 0.0,
        )
    }
}
...
impl Material for Dielectric {
    fn scatter(&self, r: Ray, hr: &HitRecord) -> (Vec3, Ray, bool) {
        let ref_ratio = if hr.front_face {
            1.0 / self.ir
        } else {
            self.ir
        };
